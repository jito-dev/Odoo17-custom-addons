...
class RevolutBillLinkLine(models.TransientModel):
    _name = 'revolut.bill.link.line'
    _description = 'Proposed Revolut transaction ↔ vendor bill link'
    _order = 'id'

    wizard_id = fields.Many2one(
        'revolut.bill.link.wizard', required=True, ondelete='cascade')
    ...
    proposed_bill_id = fields.Many2one(
        'account.move', string='Vendor Bill',
        domain="[('move_type', '=', 'in_invoice')]")
    bill_state = fields.Selection(related='proposed_bill_id.state', readonly=True)
    ...
    @api.onchange('proposed_bill_id')
    def _onchange_proposed_bill_id(self):
        # Tick "Attach" automatically when a bill is (re)selected.
        for line in self:
            line.do_attach = bool(line.proposed_bill_id)
