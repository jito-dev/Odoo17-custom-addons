...
class HpcContractorInvoice(models.Model):
    ...
    @api.model
    def _get_employee_sequence(self, employee_id):
        """Return (create on demand) an ir.sequence per employee.
        Falls back to the global CINV sequence when no employee is given."""
        if not employee_id:
            return self.env['ir.sequence'].sudo().search(
                [('code', '=', 'hpc.contractor.invoice')], limit=1
            )
        seq_code = 'hpc.ci.emp.%d' % employee_id
        ...
        if not seq:
            ...
            seq = self.env['ir.sequence'].sudo().create({
                'name': 'Contractor Invoice – %s' % (employee.name or str(employee_id)),
                'code': seq_code,
                'prefix': 'CINV/',
                'padding': 4,
                'number_next': 1,
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                seq = self._get_employee_sequence(vals.get('employee_id'))
                vals['name'] = seq.next_by_id() if seq else _('New')
