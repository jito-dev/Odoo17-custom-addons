...
"""Parallel analytic mixin for the Management Ledger (17.0.9.0.0).

A near-verbatim fork of stock ``analytic.mixin``
(odoo17_community/addons/analytic/models/analytic_mixin.py), retargeted
at the ML's own analytic models — ``jito.ledger.analytic.account`` /
``jito.ledger.analytic.plan`` — so the Management Ledger gets the exact
Stock analytic_distribution data shape (a JSON dict
``{account_id_csv: percentage}``) without sharing any record with stock
``account.analytic.*``.

Inherited by:
  * ``jito.ledger.move.line`` — analytic on ML journal-entry lines.
  * ``jito.ledger.statutory.analytic`` — analytic on the FAAP statutory
    projection, stored in a parallel side-table keyed by the stock
    ``account.move.line`` id (never writes to stock).

Unlike stock's mixin on ``account.move.line``, NOTHING here creates
analytic lines — ML analytic lines are generated explicitly by
``jito.ledger.move._create_analytic_lines`` on post.
"""
...
class JitoAnalyticMixin(models.AbstractModel):
    _name = 'jito.analytic.mixin'
    _description = 'ML Analytic Mixin'

    analytic_distribution = fields.Json(
        'Analytic Distribution',
        compute="_compute_analytic_distribution", store=True, copy=True,
        readonly=False,
    )
    ...
    distribution_analytic_account_ids = fields.Many2many(
        comodel_name='jito.ledger.analytic.account',
        compute='_compute_distribution_analytic_account_ids',
        search='_search_analytic_distribution',
    )

    def init(self):
        # GIN index for json search on the keys, only on models with a table.
    ...
    def fields_get(self, allfields=None, attributes=None):
        """Hide analytic_distribution_search from filterable fields."""
    ...
    def _compute_distribution_analytic_account_ids(self):
        ...
        existing_accounts_ids = set(Account.browse(all_ids).exists().ids)
        for rec in self:
            ids = list(unique(
                int(_id)
                for key in (rec.analytic_distribution or {})
                for _id in key.split(',')
                if int(_id) in existing_accounts_ids
            ))
            rec.distribution_analytic_account_ids = Account.browse(ids)
    ...
    def _search_analytic_distribution(self, operator, value):
        ...
        if operator == 'in' and isinstance(value, (tuple, list)):
            ...
            account_ids = list(Account._name_search(name=value, operator=operator_name_search))
    ...
    def mapped(self, func):
        if func == 'analytic_distribution' and self.env.context.get('distribution_ids'):
            return self.env['jito.ledger.analytic.account'].browse(
                flatten(record._get_analytic_account_ids() for record in self))
        return super().mapped(func)

    def filtered_domain(self, domain):
        return super(JitoAnalyticMixin, self.with_context(distribution_ids=True)).filtered_domain(domain)
    ...
    def _validate_distribution(self, **kwargs):
        if self.env.context.get('validate_analytic', False):
            Plan = self.env['jito.ledger.analytic.plan'].sudo().with_company(self.company_id)
            mandatory_plans_ids = [
                plan['id'] for plan in Plan.get_relevant_plans(**kwargs)
            ...
            for analytic_account_ids, percentage in (self.analytic_distribution or {}).items():
                for analytic_account in Account.browse(map(int, analytic_account_ids.split(","))).exists():
    ...
    def _get_analytic_account_ids(self) -> list:
        self.ensure_one()
