...
class TestHrJobBulkGemini(TransactionCase):
    """
    Test suite for the `hr.job` bulk CV processing functionality using Gemini.
    Mocks the queue_job and Gemini calls.
    """
    ...
    def setUpClass(cls):
        ...
        cls.job = cls.env['hr.job'].create({
            'name': 'Test Bulk Import Job (Gemini)',
        })
    ...
    def setUp(self):
        ...
        def mock_with_delay(self_recordset, *args, **kwargs):
            ...
            def run_job_sync(*job_args, **job_kwargs):
                ...
                real_method = getattr(self_recordset, '_process_gemini_cvs_thread')
                # Run the real method with the captured arguments
                return real_method(*job_args, **job_kwargs)

            mock_delay_obj._process_gemini_cvs_thread = MagicMock(side_effect=run_job_sync)
        ...
        # 4. Patch the `_gemini_call_for_cv` to return different responses
        # We will set this up inside each test
        self.mock_gemini_call_patcher = patch.object(
            type(self.env['hr.applicant']),
            '_gemini_call_for_cv'
        )
        self.mock_gemini_call = self.mock_gemini_call_patcher.start()
    ...
    def tearDown(self):
        ...
        self.mock_gemini_call_patcher.stop()
    ...
    def test_01_successful_bulk_process(self):
        ...
        self.mock_gemini_call.side_effect = [
        ...
        self.assertEqual(self.job.processing_failed, False)

        # 4. Check API calls
        self.assertEqual(self.mock_gemini_call.call_count, 2)
        self.mock_gemini_call.assert_any_call(self.attachment_1)
        self.mock_gemini_call.assert_any_call(self.attachment_2)
        ...
        self.mock_applicant_create.assert_any_call({
            ...
            'gemini_extract_status': _('Created from bulk import. Processing data...'),
        })
        self.mock_applicant_create.assert_any_call({
        ...
        # 8. Check processed attachments
        self.assertEqual(self.job.processed_cv_attachment_ids, self.attachment_1 + self.attachment_2)
    ...
    def test_02_partial_failure_bulk_process(self):
        ...
        self.assertEqual(self.job.processing_in_progress, False)
        self.assertEqual(self.job.processing_complete, True)
        self.assertEqual(self.job.processing_failed, True)
        ...
        # 8. Check processed attachments (only the first one)
        self.assertEqual(self.job.processed_cv_attachment_ids, self.attachment_1)
    ...
    def test_03_action_guards(self):
        ...
        # 3. Test all attachments already processed
        self.job.processed_cv_attachment_ids = [(6, 0, [self.attachment_1.id])]
        with self.assertRaises(UserError, msg="Should fail if all CVs are processed"):
            self.job.action_process_cvs()
