...
- `vendor_bill` — Service Fee / Membership: bank line → suspense, expense via a **Vendor Bill** (in_invoice) reconciled against the outflow. **The expense line is forced onto the mapped account** (`600500` Service Fees / `600510` Membership) by `_apply_mapped_expense_account` in `models/usa_bill_creation.py` — the AI extraction otherwise defaults it to the generic `600000` Expenses account.
