...
- Selection: SEPA, SWIFT, GBP, **US Bank Transfer (ACH)**, Ukrainian Bank Card, Cash, Crypto.
- Method-specific fields (IBAN, BIC, account numbers, etc.).
- **US ACH method (v1.6.0)**: `ach_recipient_name`, `ach_account_number`, `ach_routing_number` (ABA), `ach_account_type` (checking/savings), `ach_bank_name`, `ach_currency_id` (USD, readonly). A `@api.constrains` validates the routing number as **exactly 9 digits**, isolated to `method_type == 'ach'` (no other method is re-validated). ACH group is rendered in **3 views**: payment-method form, contractor form (embedded), employee portal (readonly).
