...
**Combined** (both unioned). Future versions add Management P&L,
Management Balance Sheet, and combined LL+Mgmt views via the same
...
### FX presentation translation (FR-23)
...
This implements FR-23 strictly — no FX revaluation JEs are posted; no
company-currency snapshot is ever stored on `jito.ledger.move.line`
(per HLD Decision #8).
...
Shared AbstractModel that both report handlers inherit. Hosts the
