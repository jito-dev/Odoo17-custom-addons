...
class ResCompany(models.Model):
    ...
    timesheet_rounding_enabled = fields.Boolean(
        ...
        default=False,
        help="Round tracked time (Hours Spent) to the nearest tracking step when a "
             "timesheet is saved. Entries already in the database are not affected: "
             "a duration is only ever rounded when someone enters or edits it.",
    )
    ...
    timesheet_rounding_step = fields.Selection(
        ...
        default='15',
        help="Granularity tracked time is rounded to when rounding is enabled.",
    )
