...
**It folds, and the browser does the folding.** The card is a `<details open>` whose
`<summary>` is the header. No JavaScript is involved, so it still folds on a page whose
assets failed, it is keyboard-operable for free, and the header alone — amount, due date,
"Copy details" — is enough for a customer who does not need the detail. A long invoice page
is not made longer by a block somebody has already used.
