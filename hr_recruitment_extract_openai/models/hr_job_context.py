...
class ScheduleObj(BaseModel):
    """Data structure for schedule details extracted by AI."""
...
class SalaryObj(BaseModel):
    """Data structure for salary details extracted by AI."""
...
class NotesObj(BaseModel):
    """Data structure for specific field notes extracted by AI."""
...
class JobContextExtraction(BaseModel):
    """
    Main Pydantic model representing the complete Job Context extracted 
    from a JD file or transcript.
    """
...
class HrJob(models.Model):
    ...
    def _onchange_project_budget(self):
        ...
        Reverts value if negative.
        """
        check = self._validate_non_negative(self.salary_project_budget, 'Project Budget')
        if check:
            self.salary_project_budget = check['value'] if 'value' in check else (self._origin.salary_project_budget if self._origin else 0.0)
